...
def create_framerate_csv_tex(experiment_folders, save_path):
    ...
    max_fps = df.loc[df['Algorithm'] == "No SE"]["Overall FPS"].values[0]
    min_fps = 0
    ...
    if len(results_path) == 1:
        ...
        # experiment folder containing multiple hiob executions, h_opt for example
            ...
            if mode == "" or None:
                ...
                create_graphs_from_opt_csv(results_path, fps_lim=args.fps, succ_min=args.succ_min, succ_max=args.succ_max)
            elif mode == "exp":
